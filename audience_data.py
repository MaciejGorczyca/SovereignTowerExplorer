...
def build_audiences(out_dir, quests_data, index, game_root=None):
        ...
        "stats": {
            ...
            "with_intervention": sum(1 for a in audiences.values()
                                     if any(n.startswith("Special intervention")
                                            for n in (a.get("dir") or []))),
            "with_death_followup": sum(1 for a in audiences.values()
            ...
            "with_county_intro": sum(1 for a in audiences.values() if a.get("ci")),
            "with_ultimatum": sum(1 for a in audiences.values() if a.get("um")),
    ...
    print(f"Audiences: {data['stats']['audiences']} catalogued · "
          ...
          f"{data['stats']['with_county_intro']} county introductions · "
          f"{data['stats']['with_ultimatum']} ultimatum follow-ups · "
