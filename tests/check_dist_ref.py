...
at it via EXPLORER_DIST, and runs the full test suite. The golden test's fresh
build (built into its own temp dir) is then diffed against that ref's dist.
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="dist-ref-") as tmp:
        ...
        code = subprocess.call([sys.executable, str(EXPLORER / "tests" / "run_tests.py")], env=env)
        return code
