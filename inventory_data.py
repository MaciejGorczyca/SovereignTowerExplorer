...
#
# Score is internal quest outcome points; the game UI displays ×100
# (gauge_controller.gd:103 `*100.0`, knight_score_recap.gd:21/28/37). Thus
# +1 internal ≈ +100 pts displayed, +100 internal ≈ +10,000 pts.
# Thresholds (quest.gd:29): Success >0 (>0 pts), Great Success >5 (>500 pts),
# Critical ≥10 (≥1,000 pts, shown as 2,000 pts for the total gauge).
PASSIVE_NOTES = {
    # --- special_cases.gd: score handlers (check_for_special_cases_for_score) ---
    "WISH_GRANTING_LAMP": "+100 internal (≈ +10,000 pts displayed) on every quest (special_cases.gd:25). Community reports +10k — matches displayed value.",
    "DEADLY_WEAPON": "+100 internal (≈ +10,000 pts) on Assassination (special_cases.gd:17); Efficient (+1 internal ≈ +100 pts) on Hunt / Assassination / Duel (tag_library.tscn).",
    "AMBER_EYE": "+8 internal (≈ +800 pts) on already completed quests (special_cases.gd:31).",
    "DEMON_DECOCTION": "+100 internal (≈ +10,000 pts) on already completed quests (special_cases.gd:296).",
    "SADDISTIC": "+1 internal (≈ +100 pts) when killing (special_cases.gd:93); Efficient (+1 ≈ +100 pts) on Intimidation, Inefficient (−1 ≈ −100 pts) on Help (tag_library).",
    "SERRATED_BLADE": "+1 internal (≈ +100 pts) when killing (special_cases.gd:268).",
    "GRANNYS_HERBAL_TEA": "+1/−1 internal (≈ ±100 pts) mirroring people reward (special_cases.gd:276, clamp −1..1).",
    "FINE_WINE": "+1/−1 internal (≈ ±100 pts) mirroring nobles reward (special_cases.gd:286).",
    "KIND_HEARTED": "+1/−1 internal (≈ ±100 pts) mirroring people reward (special_cases.gd:39).",
    "NOBLE_SOUL": "+1/−1 internal (≈ ±100 pts) mirroring nobles reward (special_cases.gd:49).",
    "TRUE_NOBLE_SOUL": "+1/−1 internal (≈ ±100 pts) mirroring sum of all satisfaction rewards (special_cases.gd:59).",
    "POTION_OF_FIRE_BREATHING": "+2 damage (special_cases.gd:329).",
    "PACK_OF_SERPENT_OIL_VIALS": "Funds scale with duration: bonus = 1 + Σ(i+1)/10 for i in 0..duration-1 (special_cases.gd:356).",
    # --- remaining score handlers ---
    "LONER": "+1 internal (≈ +100 pts) when sent alone (special_cases.gd:9, assigned_knights==1).",
    "COASTAL": "+1 internal (≈ +100 pts) when coastal (special_cases.gd:259).",
    "BRIZH_CONNOISSEUR": "+1 internal (≈ +100 pts) when county is brizh (special_cases.gd:250).",
    "MUTE": "−3 internal (≈ −300 pts) on Diplomacy (special_cases.gd:218).",
    "PATIENT": "+1 internal (≈ +100 pts) when duration >1 (special_cases.gd:133).",
    "TIMID": "−1 internal (≈ −100 pts) when sent alone (special_cases.gd:141).",
    "BRUTAL": "+1.5 internal (≈ +150 pts) on Hunt/Confrontation/Duel/Assassination, else −1.5 (≈ −150 pts) (special_cases.gd:158).",
    "TRUE_DRAGON_KNIGHT": "+0.5 internal (≈ +50 pts), doubled to +1.0 (≈ +100 pts) on Hunt/Confrontation/Duel/Assassination (special_cases.gd:149).",
    "REVOLUTIONAR": "Score = clamp((people−nobles)×0.2 internal ≈ ×20 pts per diff, −2..2 ≈ −200..+200 pts) (special_cases.gd:123).",
    "NOBILITY_PRIMES": "Score = clamp((nobles−people)×0.2 internal ≈ ×20 pts, −2..2 ≈ −200..+200 pts) (special_cases.gd:182).",
    "LOYAL": "Score = clamp(avg affinity×0.25 internal ≈ ×25 pts per affinity, −2..2 ≈ −200..+200 pts) with others (special_cases.gd:167).",
    "SPEEDSTER": "+0.5 internal (≈ +50 pts) per reduced duration point (special_cases.gd:192).",
    "OVERWORKED": "−0.5 internal (≈ −50 pts) per extra duration point (special_cases.gd:201).",
    "BELIEVER": "+scholars×0.025 internal (≈ ×2.5 pts per scholar) when scholars ≥10 (special_cases.gd:209).",
    "TANK": "+armor×0.08 internal (≈ ×8 pts per armor) (special_cases.gd:226).",
    "RESOURCEFULL": "+WITS×0.08 internal (≈ ×8 pts per WITS) (special_cases.gd:232).",
    "GAMBLER": "+LUCK×0.1 internal (≈ ×10 pts per LUCK) (special_cases.gd:238).",
    "PROBLEM_SOLVER": "+STRENGTH×0.1 internal (≈ ×10 pts per STR) (special_cases.gd:244).",
    # --- efficiency-only tags (tag_library.tscn) used as item passives ---
    # These give Efficient (+1 internal ≈ +100 pts) / Inefficient (−1 ≈ −100 pts) via TagLibrary.get_efficiency_tags_for_quest (quest.gd:218).
    "ABACUS_CLEAVER": "Efficient (+1 internal ≈ +100 pts displayed) on Diplomacy (tag_library.tscn).",
    "HUNTING_BOW": "Efficient (+1 internal ≈ +100 pts) on Hunt (tag_library.tscn). Community reports +100 pts correctly — internal +1.",
    "ARSENIC_VIAL": "Efficient (+1 internal ≈ +100 pts) on Assassination (tag_library.tscn). Community reports +100 pts — internal +1.",
    "ASSASSIN_DAGGER": "Efficient (+1 internal ≈ +100 pts) on Assassination (tag_library.tscn).",
    "PERFUME": "Efficient (+1 internal ≈ +100 pts) on Diplomacy (tag_library.tscn). Community reports +100 pts — internal +1.",
    "ILLICIT_PIMENTO": "Efficient (+1 internal ≈ +100 pts) when Unethical (tag_library.tscn).",
    "CRAB_ARMOR": "Efficient (+1 internal ≈ +100 pts) when Water (tag_library.tscn); also +3 armor.",
    "KRAKENS_BANE": "Efficient (+1 internal ≈ +100 pts) when Water (tag_library.tscn).",
    "SPELL_BOOK": "Efficient (+1 internal ≈ +100 pts) when Magic Involved (tag_library.tscn).",
    "DRAKE_KILLING_BOW": "Efficient (+1 internal ≈ +100 pts) when Dragon (tag_library.tscn).",
    "EYE_OF_PETRIFICATION": "Efficient (+1 internal ≈ +100 pts) on Duel (tag_library.tscn).",
    "GLASS_BLADE": "Efficient (+1 internal ≈ +100 pts) when Ghost (tag_library.tscn).",
    "DEMON_BANE": "Efficient (+1 internal ≈ +100 pts) when Ghost or At Night (tag_library.tscn).",
    "DEMONIC_SWORD": "Efficient (+1 internal ≈ +100 pts) when Magic Involved; stats scale with death_count (demonic_sword.gd): each listed stat = death_count.",
    "NET": "Efficient (+1 internal ≈ +100 pts) when Water or Big Creature (tag_library.tscn).",
    "PIKE": "Efficient (+1 internal ≈ +100 pts) when Flying Creature (tag_library.tscn).",
    "PONZI": "Efficient (+1 internal ≈ +100 pts) when Investigation (tag_library.tscn).",
    "VIRGO_SWORD": "Efficient (+1 internal ≈ +100 pts) when Crowd or Intimidation (tag_library.tscn).",
    "DURANDAL": "Efficient (+1 internal ≈ +100 pts) on Competition (tag_library.tscn).",
    "SIVKO_BURKO": "Efficient (+1 internal ≈ +100 pts) on Competition (tag_library.tscn).",
    "TROJAN": "Efficient (+1 internal ≈ +100 pts) when Cute Creature (tag_library.tscn).",
    "BUTTERMILK": "Efficient (+1 internal ≈ +100 pts) when People Involved (tag_library.tscn).",
    "PEOPLE_FRIEND": "Efficient (+1 internal ≈ +100 pts) when People Involved (tag_library.tscn).",
    "SPICED_HYPOCRAS": "Efficient (+1 internal ≈ +100 pts) on Diplomacy (tag_library.tscn).",
    "DUALIST_SWORD": "Efficient (+1 internal ≈ +100 pts) on Duel (tag_library.tscn).",
    "AGRO": "Efficient (+1 internal ≈ +100 pts) when Heavy Lifting (tag_library.tscn).",
    ...
    "KELPIE": "Duration −1 when coastal (quests_manager.gd:172), stacking with base duration_reduction 1.",
    ...
    "RISPE": "Efficient (+1 internal ≈ +100 pts) when Big Creature (tag_library.tscn).",
    ...
    "FLYING_MOUNT": "Efficient (+1 internal ≈ +100 pts) when Flying Creature, Climbing or Need Flying Mount (tag_library.tscn).",
}
